...
public class Hangman {
	...
	*/
	...
	public static String[] readWordListFile(File filename) throws IOException{
		Scanner fileReader = new Scanner(filename);
		String[] temp = new String[50];
		int lineNum=0;
		while(fileReader.hasNext()){
			temp[lineNum] = fileReader.nextLine();
			lineNum++;
		}
		String[] wordList = new String[lineNum];
	...
	*/
	...
	public static void insertGuessToArray(char guess, char[] rights, char[] wrongs, char[] secretWord){
		int rights_l = getNumWrongs(rights);
		int wrongs_l = getNumWrongs(wrongs);
		if(isLetterInWord(guess, rights)){
			return;
		}else if(isLetterInWord(guess, secretWord)){
			rights[rights_l] = guess;
		}else{
			wrongs[wrongs_l] = guess;
		}
	}
	...
	*/	
	...
	public static boolean isPlayerLose(char[] wrongs){
		if(getNumWrongs(wrongs)>=6)return true;
		else return false;
	}
